-- Enclave MVP Database Setup for Supabase
...
CREATE INDEX IF NOT EXISTS res_ft ON resource USING gin (to_tsvector('english', coalesce(title, '') || ' ' || coalesce(body, '')));
CREATE INDEX IF NOT EXISTS tag_name_idx ON tag (name);
CREATE INDEX IF NOT EXISTS resource_space_idx ON resource (space_id);
...
CREATE INDEX IF NOT EXISTS resource_updated_at_idx ON resource (updated_at);
CREATE INDEX IF NOT EXISTS query_log_space_idx ON query_log (space_id);
...
DROP TRIGGER IF EXISTS update_event_meta_updated_at ON event_meta;
...
DROP TRIGGER IF EXISTS update_gap_alert_updated_at ON gap_alert;
