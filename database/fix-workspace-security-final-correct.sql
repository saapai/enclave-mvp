-- 🔒 Fix workspace security - FINAL CORRECT VERSION
-- Based on actual column types: created_by is TEXT, auth.uid() is UUID
...
USING (
  -- Check if user created the space (created_by is TEXT, auth.uid() is UUID)
