-- 🔒 Fix workspace security - SAFE VERSION with proper type handling
-- This version avoids type casting issues by using safer comparisons
...
-- Create new policies for space table - using safer approach
...
USING (
  -- Check if user created the space (using string comparison to avoid type issues)
  (created_by IS NOT NULL AND created_by = auth.uid()::text)
  OR 
