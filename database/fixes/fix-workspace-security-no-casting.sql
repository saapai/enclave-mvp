-- 🔒 Fix workspace security - NO TYPE CASTING VERSION
-- This version avoids all type casting issues by using simpler logic
...
USING (
  EXISTS (
    ...
    AND app_user.user_id = auth.uid()::text
  )
)
WITH CHECK (
  EXISTS (
...
USING (
  -- Check if user created the space
