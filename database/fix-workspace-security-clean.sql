-- 🔒 Fix workspace security - CLEAN VERSION
-- This fixes the type casting error and creates proper RLS policies

-- First, let's disable RLS on all tables and drop all existing policies
...
BEGIN
    ...
    -- Drop policies for google_docs_chunks table
    FOR r IN (SELECT policyname FROM pg_policies WHERE tablename = 'google_docs_chunks') LOOP
        EXECUTE 'DROP POLICY IF EXISTS "' || r.policyname || '" ON google_docs_chunks';
    END LOOP;
    
    -- Drop policies for calendar_events_chunks table
    FOR r IN (SELECT policyname FROM pg_policies WHERE tablename = 'calendar_events_chunks') LOOP
        EXECUTE 'DROP POLICY IF EXISTS "' || r.policyname || '" ON calendar_events_chunks';
    END LOOP;
    
    -- Drop policies for resource_embedding table
...
-- 4. Google docs chunks policies
CREATE POLICY "gdocs_service_role" ON google_docs_chunks FOR ALL TO service_role USING (true) WITH CHECK (true);
CREATE POLICY "gdocs_authenticated" ON google_docs_chunks FOR ALL TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM app_user 
...
-- 5. Calendar events chunks policies
CREATE POLICY "calendar_service_role" ON calendar_events_chunks FOR ALL TO service_role USING (true) WITH CHECK (true);
CREATE POLICY "calendar_authenticated" ON calendar_events_chunks FOR ALL TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM app_user 
