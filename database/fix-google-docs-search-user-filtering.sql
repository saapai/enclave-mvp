-- Fix Google Docs search to include user filtering for default workspace
-- This ensures users only see their own Google Docs in personal workspace
-- Run this in Supabase SQL Editor

-- Drop and recreate the search_google_docs_vector function with user filtering
...
BEGIN
  ...
  WHERE gdc.space_id = target_space_id
    ...
    AND (target_space_id != '00000000-0000-0000-0000-000000000000' OR target_user_id IS NULL OR sgd.added_by = target_user_id)
...
-- Add comment
COMMENT ON FUNCTION search_google_docs_vector IS 'Vector similarity search for Google Doc chunks with optional user filtering for personal workspace isolation';
