-- Fix existing Google Docs tables to use TEXT for Clerk user IDs
-- Run this in your Supabase SQL editor

-- Drop existing tables if they exist (this will delete data)
DROP TABLE IF EXISTS google_doc_chunks CASCADE;
DROP TABLE IF EXISTS sources_google_docs CASCADE;
DROP TABLE IF EXISTS google_accounts CASCADE;
DROP TABLE IF EXISTS gdrive_watches CASCADE;

-- Recreate tables with correct column types
CREATE TABLE google_accounts (
  user_id TEXT PRIMARY KEY,  -- TEXT for Clerk user IDs
...
CREATE TABLE sources_google_docs (
  ...
  added_by TEXT NOT NULL,  -- TEXT for Clerk user IDs
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW(),
